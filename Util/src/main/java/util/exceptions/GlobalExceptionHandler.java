...
public class GlobalExceptionHandler {
	...
	public ResponseEntity<?> handleHttpClientException(HttpClientErrorException ex){
		return ResponseEntity.status(HttpStatus.NOT_FOUND).body(new ExceptionModel(
				ex.getMessage(), "Requested currencies not found", HttpStatus.NOT_FOUND));
	}
	
	@ExceptionHandler(MissingServletRequestParameterException.class)
	public ResponseEntity<?> handlerMissingRequestParam(MissingServletRequestParameterException ex){
		return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(
				new ExceptionModel(ex.getMessage(), "Make sure to enter all request parameters", HttpStatus.BAD_REQUEST));
	...
	public ResponseEntity<?> handleInvalidExchangeRate(NoDataFoundException ex){
		return ResponseEntity.status(HttpStatus.NOT_FOUND).body(
				new ExceptionModel(ex.getMessage(), String.format("Please make sure to enter"
						+ " currency from the list: %s", ex.getCurrencies()),
	...
	public ResponseEntity<?> handleInvalidQuantity(InvalidQuantityException ex){
		return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(
				new ExceptionModel(ex.getMessage(), "You can exchange up to 300"
						+ " units of a single currency", HttpStatus.BAD_REQUEST));
	}
